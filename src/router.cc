...
size_t Router::find_index(uint64_t ip_add) {
    ...
    for (size_t i = 0; i < rule_.size(); i++) {
        ...
        if (len == 32 || ((ip_add & rule_[i].route_prefix_) >> len) == (rule_[i].route_prefix_ >> len)) {
            if (rule_[i].prefix_length_ > rule_[p].prefix_length_) {
                p = i;
            }
        }
    }
...
//      for (size_t j = 0; j < _interfaces.size(); j++) {
//          auto que2 = interface(j)->datagrams_received();
//          std::cout << que2.size() << " ";
//      }
//      std::cout << std::endl;
  }
